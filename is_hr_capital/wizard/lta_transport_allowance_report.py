...
class WizardLtaTransport(models.Model):
    ...
    def print_report(self):
        for report in self:
            ...
            report.name = 'LTA & Transport From ' + str(from_date) + ' To ' + str(to_date)
            report_title = 'Transport Allowance From ' + str(from_date) + ' To ' + str(to_date)
            report_lta_title = 'LTA Allowance From ' + str(from_date) + ' To ' + str(to_date)
            ...
            col += 1
            excel_sheet_transport.write(row, col, 'Transport After Deduct', header_format)
            excel_sheet_transport.merge_range(0, 0, 1, 7, 'MTWA International Investment Co.LTD', title_format)
            excel_sheet_transport.merge_range(2, 0, 3, 7, report_title, title_format)
            excel_sheet_lta.merge_range(0, 0, 1, 7, 'MTWA International Investment Co.LTD', title_format)
            excel_sheet_lta.merge_range(2, 0, 3, 7, report_lta_title, title_format)
            ...
            if transport_lta_ids:
                for transport_lta_id in transport_lta_ids:
                    ...
                    excel_sheet_transport.write(row, col, sequence_id, header_format_sequence)
                    excel_sheet_lta.write(row, col, sequence_id, header_format_sequence)
                    col += 1
                    if code:
                        excel_sheet_transport.write(row, col, code, header_format_sequence)
                        excel_sheet_lta.write(row, col, code, header_format_sequence)
                    else:
                        excel_sheet_transport.write(row, col, '', header_format_sequence)
                        excel_sheet_lta.write(row, col, '', header_format_sequence)
                    col += 1
                    if employee_id:
                        excel_sheet_transport.write(row, col, employee_id, header_format_sequence)
                        excel_sheet_lta.write(row, col, employee_id, header_format_sequence)
                    else:
                        excel_sheet_transport.write(row, col, '', header_format_sequence)
                        excel_sheet_lta.write(row, col, '', header_format_sequence)
                    ...
                    col += 1
                    if department:
                        excel_sheet_transport.write(row, col, department, header_format_sequence)
                        excel_sheet_lta.write(row, col, department, header_format_sequence)
                    else:
                        excel_sheet_transport.write(row, col, '', header_format_sequence)
                        excel_sheet_lta.write(row, col, '', header_format_sequence)
                    col += 1
                    if job:
                        excel_sheet_transport.write(row, col, job, header_format_sequence)
                        excel_sheet_lta.write(row, col, job, header_format_sequence)
                    else:
                        excel_sheet_transport.write(row, col, '', header_format_sequence)
                        excel_sheet_lta.write(row, col, '', header_format_sequence)
                    col += 1
                    if transport_allowance:
                        excel_sheet_transport.write(row, col, transport_allowance, header_format_sequence)
                        total_transport_allowance += transport_allowance
                    else:
                        excel_sheet_transport.write(row, col, 0.0, header_format_sequence)
                    if lta_allowance:
                        excel_sheet_lta.write(row, col, lta_allowance, header_format_sequence)
                        total_lta += lta_allowance
                    else:
                        excel_sheet_lta.write(row, col, 0.0, header_format_sequence)
                    col += 1
                    if deduction:
                        excel_sheet_transport.write(row, col, deduction, header_format_sequence)
                        total_deduction += deduction
                    else:
                        excel_sheet_transport.write(row, col, 0.0, header_format_sequence)
                    col += 1
                    if total_transport:
                        excel_sheet_transport.write(row, col, total_transport, header_format_sequence)
                    else:
                        excel_sheet_transport.write(row, col, 0.0, header_format_sequence)
                    # total += lta_allowance
            ...
            col = 0
            row += 1
            excel_sheet_transport.merge_range(row, col, row, col + 4, 'Total', header_format)
            excel_sheet_transport.write_formula(row, 5, 'SUM(f' + str(first_row + 1) + ':f' + str(row) + ')', header_format)
            ...
            excel_sheet_transport.write_formula(row, 7, 'SUM(h' + str(first_row + 1) + ':h' + str(row) + ')',
                                                header_format)
            excel_sheet_lta.merge_range(row, col, row, col + 4, 'Total', header_format)
            excel_sheet_lta.write_formula(row, 5, 'SUM(f' + str(first_row + 1) + ':f' + str(row) + ')', header_format)
            footer_format.set_underline()
            excel_sheet_lta.write(row+2, col + 2, 'Prepared by:', footer_format)
            excel_sheet_transport.write(row+2, col + 2, 'Prepared by:', footer_format)
            footer2_format.set_text_wrap()
            excel_sheet_transport.write(row+3, col + 2, report.create_uid.name, footer2_format)
            excel_sheet_lta.write(row+3, col + 2, report.create_uid.name, footer2_format)
            excel_sheet_transport.write(row+4, col + 2, 'HR & Admin Manager', footer2_format)
            excel_sheet_lta.write(row+4, col + 2, 'HR & Admin Manager', footer2_format)
            excel_sheet_transport.write(row+4, col + 6, 'Finance & Accounting Manager', footer2_format)
            workbook.close()
            file_download = base64.b64encode(fp.getvalue())
            fp.close()
            wizard_model = self.env['lta.transport.excel']
            res_id = wizard_model.create({'name': file_name, 'file_download': file_download})
            return {
