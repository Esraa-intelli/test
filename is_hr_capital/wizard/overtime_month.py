...
class WizardOvertimeTest(models.Model):
    ...
    def print_report(self):
        for report in self:
            ...
            report.name = 'Overtime From ' + str(from_date) + ' To ' + str(to_date)
            report_title = 'Staff  Overtime' + str(from_date) + ' To ' + str(to_date)
            ...
            excel_sheet.merge_range(row, col, row+1, col, 'Name', header_format)
            col += 1
            excel_sheet.merge_range(row, col, row+1, col, 'Designation', header_format)
            col += 1
            excel_sheet.write(row, col, 'Basic', header_format)
            excel_sheet.write(row+1, col, 'Salary', header_format)
            col += 1
            excel_sheet.merge_range(row, col, row+1, col, 'Hour Value', header_format)
            col += 1
            excel_sheet.merge_range(row, col, row, col+1, 'Total Hours', header_format)
            ...
            excel_sheet.merge_range(0, 0, 1, 11, 'MTWA International Investment Co.LTD', title_format)
            excel_sheet.merge_range(2, 0, 3, 11, report_title, title_format)
            excel_sheet.merge_range(4, 0, 4, 11, str(report.create_date), header_date)
            ...
            if overtime_ids:
                ...
                for overtime_line in overtime_ids:
                    ...
                    col += 1
                    if employee_salary:
                        excel_sheet.write(row+1, col, employee_salary, contain_format)
                    else:
                        excel_sheet.write(row+1, col, '', contain_format)
                    col += 1
                    ...
                    col += 1
                    if overtime_month:
                        excel_sheet.write(row+1, col, overtime_month, contain_format)
                    else:
                        excel_sheet.write(row+1, col, '', contain_format)
                    col += 1
            ...
            row += 1
            col_8 = '8'
            excel_sheet.merge_range(row+1, col, row+1, col + 9, 'Total', header_format)
            # excel_sheet.write_formula(row+1, col + 10, 'SUM(k' + col_8.encode('utf8') + ':k' + str(row) + ')', contain_format)
            excel_sheet.write_formula(row+1, col + 10, 'SUM(k' + col_8 + ':k' + str(row) + ')', contain_format)
            ...
            excel_sheet.write(row + 3, col + 1, report.create_uid.name, footer2_format)
            excel_sheet.write(row + 4, col + 1, 'HR & Admin Manager', footer2_format)
            excel_sheet.write(row + 2, col + 6, ' Approved by: ', footer_format)
            excel_sheet.write(row + 4, col + 6, 'Finance & Accounting Manager', footer2_format)
            workbook.close()
